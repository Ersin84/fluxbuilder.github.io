self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var C={ajT:function ajT(d,e,f,g,h,i,j){var _=this
_.c=d
_.e=e
_.f=f
_.r=g
_.x=h
_.y=i
_.a=j},c7r:function c7r(d){this.a=d},
f5I(d,e,f,g){return new C.a6A(e,d,f,!0,null)},
a6A:function a6A(d,e,f,g,h){var _=this
_.c=d
_.d=e
_.e=f
_.f=g
_.a=h},
aHp:function aHp(d){this.a=null
this.b=d
this.c=null},
cD8:function cD8(d){this.a=d},
cD6:function cD6(d){this.a=d},
cD7:function cD7(d){this.a=d},
cD9:function cD9(d,e){this.a=d
this.b=e},
cDb:function cDb(d,e,f){this.a=d
this.b=e
this.c=f},
cDa:function cDa(d,e,f){this.a=d
this.b=e
this.c=f},
eEy(a0,a1){var x,w,v,u,t,s,r,q,p,o,n,m,l,k,j,i,h="TabBarConfig",g=null,f="VerticalLayout",e="SplashScreen",d=A.y(a1,!1,y.k)
try{x=a0.h(0,"Setting")
w=a0.h(0,"Drawer")
for(q=J.De(J.Gb(x)),p=q.length,o=0;o<q.length;q.length===p||(0,A.X)(q),++o){v=q[o]
d.aTr(v,J.f(x,v))}if(J.f(x,h)==null)d.aTr(h,g)
A.U("--updated setting",g)
if(w!=null){for(q=J.De(J.Gb(w)),p=q.length,o=0;o<q.length;q.length===p||(0,A.X)(q),++o){u=q[o]
d.cbA(u,J.f(w,u))}A.U("--update drawer",g)}q=a0.h(0,"AppBar")
if(q==null){q=y.z
q=A.G(q,q)}p=y.N
n=y.z
t=A.ba(q,p,n)
q=d
m=A.ba(t,p,n)
l=q.gaN()
k=q.gaN().d
l.d=k==null?g:k.aJz(m)
q.L()
A.U("--updated appbar",g)
q=y.a
d.WG(A.ejs(A.b8(a0.h(0,"TabBar"),!0,q)))
A.U("--updated tabbar",g)
l=a0.h(0,"HorizonLayout")
d.Jr(A.e5f(A.b8(l==null?[]:l,!0,q)))
A.U("--updated list horizontal widget",g)
q=a0.h(0,"Background")
s=A.ba(q==null?A.G(n,n):q,p,n)
d.a7u(s)
A.U("--updated background",g)
if(a0.h(0,f)!=null){d.a7E(A.ba(A.ejt(a0.h(0,f)),p,n))
A.U(a0.h(0,f),g)}else{q=d
l=q.gaN().d
if(l!=null)l.a=A.G(p,n)
q.L()}A.U("--updated vertical widget",g)
if(a0.h(0,e)!=null){q=d
l=A.ba(a0.h(0,e),p,n)
k=q.gaN()
j=q.gaN().d
k.d=j==null?g:j.bSZ(new A.a50(A.ba(l,p,n),y.Y))
q.L()}else{q=d
l=q.gaN().d
if(l!=null)l.d=A.G(p,n)
q.L()}A.U("--updated splash screen widget",g)}catch(i){r=A.a_(i)
A.U(J.J(r),g)}}},A,B,D,J
a.setFunctionNamesIfNecessary([C])
C=a.updateHolder(c[5],C)
A=c[0]
B=c[2]
D=c[58]
J=c[1]
C.ajT.prototype={
c98(d){var x,w,v,u,t,s=this,r=null,q="keywords",p=s.r,o=p*2,n=y.p,m=A.b([A.x(r,A.Dv(r,new A.cB(new C.c7r(s),r),r,r,B.P,r,B.T,r,o,r,r,p),B.e,r,r,r,r,r,r,D.aH1,r,r,r,r)],n)
if(s.c){x=A.j(d).w.Q
w=A.j(d).d
w=A.S(245,w.gj(w)>>>16&255,w.gj(w)>>>8&255,w.gj(w)&255)
v=A.ag(9)
u=s.y
t=A.d(u.e,"typeFramework")
t=A.b([A.P(A.b([A.u(t[0].toUpperCase()+B.d.c0(t,1),r,r,r,r,r,A.j(d).w.x.ed(B.R),r,r,r),A.P4(d,r,r,"The Compatible Product that you have purchased.\nOne product will compatible with many templates")],n),B.k,r,B.aw,B.h,r,r),B.B,B.kw,A.P3(d,"Device"),A.u("iOS, Android, PWA",r,r,r,r,r,x,r,r,r)],n)
u=u.f
if(A.d(u,q).length!==0)B.b.F(t,A.b([B.kw,A.P3(d,"App Type"),A.u(A.d(u,q),r,r,r,r,r,x,r,r,r)],n))
t.push(B.aB)
n=A.HZ(r,r,A.j(d).b,r,r,r,r,r,r,B.L,B.eu,B.L,r,new A.fx(A.ag(6),B.a5),r,r,r,r)
t.push(A.dG(!1,A.P(D.cQS,B.k,r,B.af,B.h,r,r),B.e,r,r,r,r,r,s.f,n))
m.push(A.x(r,A.L(t,B.t,r,B.j,B.h,r),B.e,r,r,new A.Z(w,r,r,v,r,r,r,B.p),r,50+o,r,r,B.au,r,r,p+60))}return A.b5(B.I,m,B.v,B.J,r,r)},
p(d,e){var x=null,w=A.ag(9),v=A.j(e).d
v=A.S(B.f.I(127.5),v.gj(v)>>>16&255,v.gj(v)>>>8&255,v.gj(v)&255)
return A.ax(x,A.x(x,A.Km(this.c98(e),B.T,x,B.P,A.Qq()),B.e,x,x,new A.Z(v,x,x,w,x,x,x,B.p),x,x,x,B.j8,x,x,x,x),B.n,!1,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,this.e,x,x,x,x,x,x,x,x)}}
C.a6A.prototype={
B(){return new C.aHp(B.m)},
c4Z(){return this.c.$0()},
ams(d){return this.d.$1(d)}}
C.aHp.prototype={
bN(d){var x,w
this.Kl(d)
x=$.D7()
w=this.c
w.toString
x.pi(A.y(w,!1,y.k).gaN().d.U())},
bAl(){var x=this.c
x.toString
A.D6(B.al,!0,new C.cD8(this),x,null,!0,!0,y.z)},
PJ(d,e){return this.b1h(d,e)},
b1h(d,e){var x=0,w=A.t(y.H),v
var $async$PJ=A.o(function(f,g){if(f===1)return A.p(g,w)
while(true)switch(x){case 0:v=d.c
A.d($.aB().aG$,"api").c=A.d(v,"mockFolder")
x=2
return A.l($.DB().$1$0(y.C).hJ("String","mockFolder",A.d(v,"mockFolder")),$async$PJ)
case 2:$.D7().pi(e)
return A.q(null,w)}})
return A.r($async$PJ,w)},
RW(d,e){return this.bCb(d,e)},
bCb(d,e){var x=0,w=A.t(y.H),v=this,u,t,s,r
var $async$RW=A.o(function(f,g){if(f===1)return A.p(g,w)
while(true)switch(x){case 0:r=$.f8;(r==null?$.f8=A.GO():r).a=null
$.CZ().a.E(0,new A.VO(0,!1))
r=v.c
r.toString
u=y.D
r=A.y(r,!1,u)
r.d=d
r.c=e
r.L()
x=2
return A.l(e.dm(),$async$RW)
case 2:t=g
r=v.c
r.toString
A.y(r,!1,u).b=t
r=v.c
r.toString
r=A.y(r,!1,u).f
s=v.c
x=r?3:5
break
case 3:s.toString
r=A.y(s,!1,u)
r.f=!1
r.L()
A.ci(B.bi,new C.cD9(v,e),y.P)
x=4
break
case 5:s.toString
x=6
return A.l(v.PJ(e,A.y(s,!1,u).WX()),$async$RW)
case 6:r=v.c
r.toString
r=A.y(r,!1,y.k)
r.d=A.cn(10)
r.L()
case 4:v.a.c4Z()
return A.q(null,w)}})
return A.r($async$RW,w)},
p(d,e){var x,w,v=null,u=this.a.e
if(u==null)return A.x(v,v,B.e,v,v,v,v,v,v,v,v,v,v,v)
x=y.w
w=e.D(x).f.a.giK()<600?e.D(x).f.a.a*0.4:120
return A.Km(A.eL(B.aa,A.dS(J.a9(A.d(u.d,"templates")),new C.cDb(this,u,w),!0,y.l),B.b6,B.F,B.aa,20,0,v,B.a8),B.at,v,B.P,A.Qq())}}
var z=a.updateTypes(["~()","ajT(F)"])
C.c7r.prototype={
$2(d,e){var x=null,w=this.a,v=w.r,u=A.ag(3)
return A.aG(A.x(x,A.fh(A.d(w.y.b,"image"),B.r,300,x,B.eL,x,"core_builder",x),B.e,x,x,new A.Z(x,x,x,u,x,x,x,B.p),x,v*2,x,x,B.j9,x,x,v),x,x,x)},
$S:1931}
C.cD8.prototype={
$1(d){var x,w=null,v=A.x(w,D.dMz,B.e,w,w,w,w,w,w,w,D.aH_,w,w,380),u=A.dG(!1,B.ys,B.e,w,w,w,w,w,new C.cD6(d),w),t=this.a,s=t.c
s.toString
s=A.j(s)
x=t.c
x.toString
return A.Ez(A.b([u,A.dG(!1,D.dM9,B.e,w,w,w,w,w,new C.cD7(t),A.HZ(w,w,s.b,w,w,w,w,w,w,w,w,A.j(x).q.c,w,w,w,w,w,w)),B.cy],y.p),B.D,w,w,B.e,v,B.cw,w,B.bT,!1,w,w,w,w,w)},
$S:41}
C.cD6.prototype={
$0(){A.a7(this.a,!1).b3(0,null)
return null},
$S:0}
C.cD7.prototype={
$0(){var x,w,v,u,t=this.a,s=t.a
s.toString
x=t.c
x.toString
w=y.D
x=A.y(x,!1,w).c
s.ams(x==null?null:A.d(x.e,"typeFramework"))
s=t.c
s.toString
v=A.agn(A.ba(A.y(s,!1,w).WX(),y.N,y.z))
w=t.c
w.toString
C.eEy(v.U(),w)
w=t.c
w.toString
A.cI4(w,"customize")
s=t.c
s.toString
A.y(s,!1,y.k).gaN().a8S()
s=t.c
s.toString
u=A.Tl(s)
if(u!=null&&u.kk()){t=t.c
t.toString
A.a7(t,!1).b3(0,null)}},
$S:0}
C.cD9.prototype={
$0(){var x=0,w=A.t(y.P),v=this,u,t
var $async$$0=A.o(function(d,e){if(d===1)return A.p(e,w)
while(true)switch(x){case 0:u=v.a
t=u.c
t.toString
x=2
return A.l(u.PJ(v.b,A.y(t,!1,y.D).WX()),$async$$0)
case 2:u=u.c
u.toString
u=A.y(u,!1,y.k)
u.d=A.cn(10)
u.L()
return A.q(null,w)}})
return A.r($async$$0,w)},
$S:42}
C.cDb.prototype={
$1(d){var x,w,v,u=this.b,t=J.f(A.d(u.d,"templates"),d),s=this.a,r=s.c
r.toString
x=y.D
if(A.y(r,!1,x).c==null)w=!1
else{r=A.d(t.a,"id")
v=s.c
v.toString
v=A.y(v,!1,x).c
if(r===(v==null?null:A.d(v.a,"id"))){r=A.d(u.a,"id")
v=s.c
v.toString
x=r===A.y(v,!1,x).d
w=x}else w=!1}s.a.toString
return new C.ajT(w,new C.cDa(s,t,u),s.gbAk(),this.c,!0,t,null)},
$S:z+1}
C.cDa.prototype={
$0(){return this.a.RW(A.d(this.c.a,"id"),this.b)},
$S:7};(function installTearOffs(){var x=a._instance_0u
x(C.aHp.prototype,"gbAk","bAl",0)})();(function inheritance(){var x=a.inherit,w=a.inheritMany
x(C.ajT,A.Y)
x(C.c7r,A.QI)
x(C.a6A,A.M)
x(C.aHp,A.Vf)
w(A.Ea,[C.cD8,C.cDb])
w(A.NI,[C.cD6,C.cD7,C.cD9,C.cDa])})()
A.eS(b.typeUniverse,JSON.parse('{"ajT":{"Y":[],"k":[]},"a6A":{"M":[],"k":[]},"aHp":{"N":["a6A"]}}'))
var y=(function rtii(){var x=A.ab
return{k:x("DY"),p:x("K<k>"),a:x("a2<n,@>"),w:x("Db"),P:x("aI"),Y:x("a50<a2<n,@>>"),C:x("Ru"),N:x("n"),D:x("RD"),l:x("k"),z:x("@"),H:x("~")}})();(function constants(){var x=a.makeConstList
D.aH_=new A.V(30,10,30,10)
D.aH1=new A.V(30,25,30,25)
D.cIf=new A.bd(B.Cd,13,B.w,null)
D.dNu=new A.at("Apply Now",null,B.yf,null,null,null,null,null,null,null,null,null,null,null,null)
D.cQS=A.b(x([D.cIf,B.cy,D.dNu]),y.p)
D.dM9=new A.at("YES",null,B.dy,null,null,null,null,null,null,null,null,null,null,null,null)
D.dMz=new A.at("All your previous customized design will be override by this template layout?",null,null,null,null,null,null,null,null,null,null,null,null,null,null)})()}
$__dart_deferred_initializers__["8R2fU5ts6VlE4/oQqJUASn6fvUQ="] = $__dart_deferred_initializers__.current
